...
export async function encryptChunk(chunk: Uint8Array, keyBase64: string, nonceBase64: string): Promise<Uint8Array> {
  ...
  const cryptoKey = await crypto.subtle.importKey("raw", keyBytes as unknown as BufferSource, ALGORITHM, false, ["encrypt"]);
  const encryptedBuffer = await crypto.subtle.encrypt(
    { name: ALGORITHM, iv: nonce as unknown as BufferSource },
    cryptoKey,
    chunk.slice() as unknown as BufferSource,
  );
...
export async function decryptChunk(encryptedChunk: Uint8Array, keyBase64: string, nonceBase64: string): Promise<Uint8Array> {
  ...
  const cryptoKey = await crypto.subtle.importKey("raw", keyBytes as unknown as BufferSource, ALGORITHM, false, ["decrypt"]);
  const decryptedBuffer = await crypto.subtle.decrypt(
    { name: ALGORITHM, iv: nonce as unknown as BufferSource },
    cryptoKey,
    encryptedChunk.slice() as unknown as BufferSource,
  );
...
async function _getKeyFromPassword(password: string): Promise<CryptoKey> {
  ...
  return crypto.subtle.deriveKey(
    { name: "PBKDF2", salt: encoder.encode("storeit-telegram-salt") as unknown as BufferSource, iterations: 100000, hash: "SHA-256" },
    keyMaterial,
...
export async function encryptFile(file: File, _password: string): Promise<{ encrypted: Blob; iv: string; key: string }> {
  ...
  const encryptedBuffer = await crypto.subtle.encrypt(
    ...
    await crypto.subtle.importKey("raw", keyBytes as unknown as BufferSource, ALGORITHM, false, ["encrypt"]),
    buffer
  );
...
export async function decryptFile(encryptedBlob: Blob, iv: string, key: string): Promise<Blob> {
  ...
  const decrypted = await crypto.subtle.decrypt({ name: ALGORITHM, iv: ivBytes as unknown as BufferSource }, cryptoKey, buffer);
